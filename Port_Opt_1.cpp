...
void Metropolis () {
    ...
    double realVar = (VarF(RealMVP, V));
    while (E > .4554155) { //.4544155 is known beforehand to be optimal variance
...
void Proposal () { // keep in mind the constraints as well as the concepts of neighbors
    ...
    for (i = 1; i < 51; ++i) {
        // copies the stuff in port x into y, we then make changes to y to create a proposal
        porty[i][1] = portx[i][1];
    }
    ...
    porty[i][1] -= .002; //subtracts one cent from a stock randomly chosen
    porty[j][1] += .002; //adds one cent to a stock randomly chosen
